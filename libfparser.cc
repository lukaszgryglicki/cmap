...
void err_exception(const char* reason)
{
...
void fpar_info()
{
 printf("supported functions: sin,cos,tan,tg,ctan,ctg,exp,ln,log,sqrt,abs\n"
        ...
        "supported functions: real,imag,conj,abs,arg,i\n"
...
complex<double> factor()
{
 ...
 if (isdigit(ch) || ch=='.')
   {
    buffer[--position] = ch;
    sscanf(buffer+position, "%lf%c", &f_r, &ch);		//check
    f = complex<double>(f_r, 0.0);
    c=ch;
    do read_next_char();
    while (ch != c);
   }
 ...
 else
   {
    ...
    else if (!strcmp(ident, "i")) f = complex<double>(0., 1.);
	...
	if (ch=='(')
	  {
	   buffer[--position] = ch;
	   f = complex<double>(exponential().real(), 0.);
	  }
	else err_exception("Expected: '(' after real.\n");
      }
    else if (!strcmp(ident, "imag"))
      {
	skipblanks();
	if (ch=='(')
	  {
	   buffer[--position] = ch;
	   f = complex<double>(0., exponential().imag());
	  }
	else err_exception("Expected: '(' after imag.\n");
      }
    else if (!strcmp(ident, "conj"))
      {
	skipblanks();
	if (ch=='(')
	  {
	...
	else err_exception("Expected: '(' after conj.\n");
      }
	...
	if (ch=='(')
	...
	else if (!strcmp(ident, "arg"))
	  {
	   ...
	   else err_exception("Expected: '(' after arg.\n");
	  }
